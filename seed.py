...
#imported db from model which is SQL Alchemy
...
#         if title.endswith(')'):
#             title = title[:-7]
#         #another way to trim:   title[:len(" (YEAR)")]
...
#         add_movie = Movie(movie_id=movie_id, title=title, released_at=released_at, imdb_url=imdb_url)
